...
StatusPulse is a lightweight uptime and service monitoring application written in Go. Phase 3 adds periodic HTTP checks and recent check history to the service management API.

Services and check history are stored in memory and lost whenever the application stops. SQLite persistence comes in Phase 4. Run this version locally or on a trusted private network; registered URLs cause outbound requests, including to private addresses.
...
History retains the most recent 1,000 checks per service; `limit` accepts 1–100
